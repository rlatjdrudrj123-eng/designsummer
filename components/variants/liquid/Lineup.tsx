...
/* [LIQUID 변형] 원본 components/sections/Lineup.tsx 클론 — 구조·카피·인터랙션 동일.
...
export default function Lineup({ day }: { day: 1 | 2 }) {
  ...
  const track = day === 1 ? conference.about.days[0] : conference.about.days[1];
  ...
  return (
    ...
    >
      <Reveal className={styles.head}>
        <p className={styles.headDay}>
          <span className={styles.headLabel}>Day {day}</span>
          <span className={styles.headDate}>{track.date}</span>
        </p>
        <h2 className={styles.headTitle}>{track.title}</h2>
      </Reveal>
      {list.map((s, i) => (
