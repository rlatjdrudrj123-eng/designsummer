...
/* 히어로 열 필드 (가이드 3·4·8장) — 자동 캘리브레이션 정지 상태 기반.
   필드 = 가우시안 열원(양수) + 흰 만(음수)의 밀도 합 + 저주파 도메인 워프.
   파라미터는 scripts/calibrate-heat.mjs 가 kv.png 에 피팅해 content/heat-field.json 에 저장.
   여기서는 그 정지 상태를 렌더하고, 소스별 미세 드리프트 + 커서 소스(=find your heatmap)를
   더한다. 저해상 버퍼에서 계산 후 업스케일(매끈함). reduced-motion: 드리프트·커서 정지. */
...
const LUT = Uint8ClampedArray.from(field.lut);
const TRAIL_MS = 1100;
